...
(function() {
  ...
  function getBaseUrl() {
    try {
      ...
      for (let i = 0; i < scripts.length; i++) {
        ...
        if (src && src.includes('intercom-style-widget-adapter.js') || src.includes('widget-loader.js')) {
  ...
  function loadIntercomStyleWidget() {
    ...
    try {
      ...
      // Use relative path with the determined base URL
      script.src = baseUrl + 'intercom-style-widget.js?widgetId=' + encodeURIComponent(widgetId);
      ...
      console.error('DEBUG: Adapter - Error in loadIntercomStyleWidget:', error);
      tryAlternativeLoading();
    }
  }
  ...
  function tryAlternativeLoading() {
    try {
      ...
      const script = document.createElement('script');
      script.id = 'intercom-style-widget-script-fallback';
